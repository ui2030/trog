...
- **아이템 발견**: `[이름 획득: 아이템명 | 종류 | 효과]`
  · 시체에서: `[허접 획득: 녹슨 단검 x1 | 무기 | 공격 +2]`, `[허접 획득: 동전 주머니 | 소모품 | 골드 +N]`
...
- "고블린의 허리띠에서 [허접 획득: 동전 주머니 | 소모품 | 골드 +12] 가 떨어졌다."
...
def parse_and_apply_monsters(text: str, monsters: "Dict[str, Monster]",
    ...
    for m in MONSTER_SPAWN_PATTERN.finditer(text):
        ...
        if name not in monsters:
            ...
            print(f"[MONSTER SPAWN DUP] {name!r} already exists in room, ignored. "
                  f"(DM 은 동명이인 적 만들 때 'A/B/C' 또는 '우두머리' 식 구분 필요)")
    ...
    for m in ITEM_PATTERN.finditer(text):
        ...
        if kind == "equipment":
            ...
            if corrected and corrected != slot:
                print(f"[ITEM SLOT FIX] {item!r}: {slot} → {corrected} (이름 기반 보정)")
                slot = corrected
        target = _match_player(name_field, players)
        if not target:
            continue
        ...
        auto_equipped = False
        if kind == "equipment" and slot == "dual" and qty == 1:
            # 양손에 동일 아이템 (이름 그대로). off_hand 에 들어가있던 것은 인벤으로 회수됨.
            replaced_main = target.equip_to_slot("main_hand", item, effect)
            replaced_off  = target.equip_to_slot("off_hand", item, effect)
            auto_equipped = True
            gained.append({
                "name": target.name, "item": item, "effect": effect,
                "quantity": 1, "kind": kind, "slot": "dual",
                "auto_equipped": True,
                "replaced": replaced_main,
                "replaced_off": replaced_off,
            })
        elif kind == "equipment" and slot in ("main_hand", "off_hand", "armor", "accessory") and qty == 1:
...
def parse_and_upgrade_equipment(text: str, players: Dict[str, "Player"]) -> List[dict]:
    ...
    for m in EQUIP_UPGRADE_PATTERN.finditer(text):
        ...
        if not prev_name:
            print(f"[UPGRADE SKIP] {target.name} slot {slot} 비어있음 — 강화 무시")
            continue
        ...
        dual_synced = False
        if slot == "main_hand":
            oh = target.equipped.get("off_hand") or {}
            if isinstance(oh, dict) and oh.get("name") == prev_name:
