...
# ── 패키지 Q: DM 텍스트 품질 게이트 ─────────────────────────────
# 무료 중계 모델이 한국어 붕괴/영단어 혼입 응답을 내면 히스토리에 들어가 다음 응답이
# 이를 모방(오염 루프)한다. 게이트는 붕괴 응답을 탐지해 1회 재생성 + 히스토리에서 격리한다.
_DM_QUALITY_MIN_BODY_LEN = 80        # 본문 이보다 짧으면 무조건 pass (짧은 응답 오탐 방지)
_DM_QUALITY_MAX_LATIN_WORDS = 5      # 라틴 4자+ 연속 단어 이 개수 '초과' 시 fail
_DM_QUALITY_MIN_HANGUL_RATIO = 0.75  # 한글/(한글+라틴) 이 미만이면 fail
_DM_QUALITY_RETRY_NOTE = "직전 응답이 손상되어 재생성한다. 반드시 자연스러운 한국어로만 서술하라."
_DM_QUALITY_TAG_PATTERN = re.compile(r'\[[^\]]*\]')      # [🎬 SCENE:], [🎲DM d20:], [진행:] 등 모든 대괄호 태그·뱃지
_DM_QUALITY_MARKDOWN_PATTERN = re.compile(r'[*_`#>~]')
_DM_QUALITY_LATIN_WORD_PATTERN = re.compile(r'[A-Za-z]{4,}')


def _dm_text_quality_ok(text: str) -> Tuple[bool, str]:
    """DM 본문이 한국어로 성립하는지 판정하는 순수 함수. (ok, reason) 반환.
    태그·마크다운 기호·숫자를 제거한 본문만 평가한다. 레버는 모듈 상수."""
    if not text:
        return True, ""
    body = _DM_QUALITY_TAG_PATTERN.sub(' ', text)
    body = _DM_QUALITY_MARKDOWN_PATTERN.sub(' ', body)
    body = re.sub(r'\d+', ' ', body)
    if len(body.strip()) < _DM_QUALITY_MIN_BODY_LEN:
        return True, ""
    if len(_DM_QUALITY_LATIN_WORD_PATTERN.findall(body)) > _DM_QUALITY_MAX_LATIN_WORDS:
        return False, "latin"
    hangul = sum(1 for ch in body if '가' <= ch <= '힣')
    latin = sum(1 for ch in body if 'a' <= ch.lower() <= 'z')
    denom = hangul + latin
    if denom > 0 and (hangul / denom) < _DM_QUALITY_MIN_HANGUL_RATIO:
        return False, "hangul_ratio"
    return True, ""


async def _dm_complete_checked(system: str, messages: List[dict], max_tokens: int,
                               on_chunk=None, where: str = "") -> Tuple[str, bool]:
    """DM 서사 생성 + 품질 게이트. fail 시 1회만 재시도(시스템 프롬프트 말미 1줄 추가, non-stream).
    재시도도 fail이면 그 텍스트 그대로 사용하고 (text, quality_bad=True) 반환 — 게임은 멈추지 않는다.
    재시도가 non-stream인 이유: 1차 partial 은 이미 클라에 broadcast 됐고, 클라는 최종 dm_response
    도달 시 partial placeholder 를 정식 텍스트로 교체하므로(game.js dm_response 핸들러) 재스트림 불필요."""
    text = await llm_complete(system, messages, max_tokens=max_tokens, on_chunk=on_chunk)
    ok, reason = _dm_text_quality_ok(text)
    if ok:
        return text, False
    retry = await llm_complete(system + "\n\n" + _DM_QUALITY_RETRY_NOTE, messages, max_tokens=max_tokens)
    ok2, reason2 = _dm_text_quality_ok(retry)
    if ok2:
        return retry, False
    logger.warning("[DM_QUALITY] %s: 재시도 후에도 품질 미달(1차:%s, 2차:%s) — 그대로 사용",
                   where or "?", reason, reason2)
    return retry, True


def _dm_assistant_msg(text: str, quality_bad: bool) -> dict:
    """assistant 히스토리 메시지 dict. quality_bad면 플래그를 달아 _llm_slice 가 제외하게 한다."""
    msg = {"role": "assistant", "content": text}
    if quality_bad:
        msg["quality_bad"] = True
    return msg
...
**같은 종류 적이 여럿이면 반드시 첫 개체부터 A/B/C 로 구분해 명명하라** (예: "고블린 A", "고블린 B") — 같은 이름으로 여러 마리를 찍으면 개별 관리가 깨진다.
...
def build_system_prompt(scenario_id: Optional[str]) -> str:
    ...
    return (
        DM_SYSTEM_PROMPT
        + "\n본문은 자연스러운 한국어로만 쓴다. 영어 단어·로마자를 섞지 않는다(고유 지명·인명도 한글 표기).\n"
...
ITEM_KIND_KEYWORDS = {
    ...
    # O-3: 슬롯 미지정 generic '장비' → slot None. 이름 힌트(_NAME_SLOT_HINTS)로도 슬롯을 못 정하면
    # main_hand 로 때려넣지 않고 인벤토리 보관만 한다(사용자가 수동 장착). 무기 칸 오장착 방지.
    "장비":     ("equipment", None),
    "소모품":   ("consumable", None),
    "퀘스트":   ("quest", None),
    "키":       ("quest", None),
...
_NAME_SLOT_HINTS = [
    ...
    # 방어구 슬롯 — 갑옷·로브·투구·신발·장갑·의류 모두 단순화해 armor 로
    # O-3: 의류 계열(옷·튜닉·의복·조끼·코트·갑주·경갑·중갑) 추가. "옷감"(제작 재료)은 부정 전방탐색으로 제외.
    (re.compile(r"갑옷|갑주|경갑|중갑|로브|튜닉|의복|조끼|코트|흉갑|체인메일|판금|가죽갑|사슬갑|투구|헬름|신발|부츠|장화|장갑|건틀릿|망토|옷(?!감)"), "armor"),
    # 장신구
...
# [P-1] 자동 장착 폐지 — 획득물은 전부 인벤토리로. 장착은 사용자 명시 행동(P-2 슬롯/장착 UI)만.
...
def _next_monster_suffix(base: str, monsters: "Dict[str, Monster]") -> Optional[str]:
    """동명 스폰 시 'base B','base C' … 중 아직 없는 첫 이름 반환. B~Z 소진 시 None."""
    for c in "BCDEFGHIJKLMNOPQRSTUVWXYZ":
        cand = f"{base} {c}"
        if cand not in monsters:
            return cand
    return None
...
def parse_and_apply_monsters(text: str, monsters: "Dict[str, Monster]",
    ...
    def _find(raw_name: str) -> Optional["Monster"]:
        ...
        if len(candidates) == 1:
            return candidates[0]
        if len(candidates) > 1:
            # O-2: 후보 다수 — 조용히 증발(None)시키는 대신 '살아있는 것 중 가장 먼저 등장한 개체'를 선택.
            # ponytail: dict 삽입순 = 스폰순. 완벽하진 않음(DM 이 특정 개체를 의도했을 수 있음) — 로그로 보완.
            alive = [c for c in candidates if c.hp > 0]
            pick = (alive or candidates)[0]
            logger.warning("[MONSTER FIND] %r 부분일치 후보 %d개(%s) → %r 선택",
                           n, len(candidates), [c.name for c in candidates], pick.name)
            return pick
        return None
    ...
    def _apply_hp_change(raw_name: str, new_hp: int) -> bool:
        ...
        if acting_player_id and target.hp < prev_hp:
            ...
            target.last_damager_id = acting_player_id  # O-1: 실제 데미지 딜러(DOT 처치자 폴백용)
        events.append({"kind": "hp", "name": target.name, "hp": target.hp, "max_hp": target.max_hp})
        if target.hp <= 0:
            # 처치 시 attackers 스냅샷을 이벤트에 동봉 — 호출부에서 XP 분배.
            # O-1: killer = 이번 턴 행동자(acting_player_id). attackers 엔 어시스트 후보 전원이 담겨
            # attackers[-1] 이 더 이상 처치자가 아니므로 killer 를 명시적으로 싣는다.
            events.append({
                "kind": "defeated",
                "name": target.name,
                "max_hp": target.max_hp,
                "attackers": list(target.attackers),
                "killer": acting_player_id,
            })
    ...
    for m in MONSTER_SPAWN_PATTERN.finditer(text):
        ...
        if name not in monsters:
            ...
            # O-2: 같은 이름 재spawn — 무시하면 고블린×3 이 1마리로 붕괴한다.
            # 두 번째부터 자동 접미사(B,C,…)를 붙여 개별 개체로 등록·브로드캐스트.
            suffixed = _next_monster_suffix(name, monsters)
            if suffixed:
                monsters[suffixed] = Monster(suffixed, hp, speed=speed)
                events.append({"kind": "spawn", "name": suffixed, "hp": hp, "speed": speed})
                logger.info("[MONSTER SPAWN DUP] %r already exists → registered as %r", name, suffixed)
            else:
                logger.warning("[MONSTER SPAWN DUP] %r 접미사 소진(B~Z), ignored.", name)
...
_AMBUSH_INTENT = re.compile(r"(매복|기습|몰래\s*(다가|접근|덮|친다)|기척을\s*죽이|숨어서\s*(친다|공격|덮|기습)|은신하?여?\s*(공격|덮|친다))")
def _is_ambush_action(text: str) -> bool:
    return bool(_AMBUSH_INTENT.search(text or ""))

# 🆕 O-4: 말로 탐색 시작 — 강한 의도만 통과시키는 보수적 휴리스틱.
#   '탐색/수색/정찰' 직접 포함, 또는 '둘러/살펴' + 공간어(주변/근처/일대/지역) 조합.
# '반지를 살펴본다' 같은 대상 관찰은 공간어가 없어 배제된다(완벽 불가 — 보수적 목록으로 시작).
def _is_explore_action(text: str) -> bool:
    t = text or ""
    if re.search(r"탐색|수색|정찰", t):
        return True
    return bool(re.search(r"둘러|살펴", t) and re.search(r"주변|근처|일대|지역", t))
...
def parse_and_apply_items(text: str, players: Dict[str, "Player"],
    ...
    🆕 [P-1] 자동 장착 폐지 — 모든 획득물은 인벤토리로만 들어간다(auto_equipped 항상 False).
    kind/slot 분류는 유지(장착 UI 가 슬롯 매칭에 사용).
    """
    gained: List[dict] = []
    item_gains = 0       # 실제 인벤 획득 카운트 (통화→골드 변환은 제외)
    for m in ITEM_PATTERN.finditer(text):
        name_field = m.group(1).strip()
        ...
        # 🆕 [P-1] 자동 장착 전면 폐지 — 획득 아이템은 무조건 인벤토리(소지품)로.
        # 장비도 예외 없이 보관만 하고, 장착은 사용자가 슬롯/장착 UI에서 명시적으로 할 때만(P-2).
        # kind/slot 분류는 그대로 유지 — 장착 UI(guessEquipSlot 미러)가 슬롯 매칭에 씀.
...
class Player:
    ...
    def unequip_slot(self, slot: str) -> Optional[dict]:
        """🆕 [P-2] 슬롯의 장비를 벗어 인벤토리(kind=equipment)로 회수.
        양손 동일 무기(쌍)면 두 손 모두 비우고 한 묶음만 회수(refreshCharPanel isDual 규약과 동일).
        반환: 회수된 장비 {name, effect} 또는 None(빈 슬롯 → no-op)."""
        slot = _SLOT_ALIASES.get(str(slot).strip().lower(), slot)
        if slot not in self.equipped:
            return None
        cur = self.equipped.get(slot) or {}
        name = cur.get("name") if isinstance(cur, dict) else None
        if not name:
            return None
        effect = cur.get("effect") if isinstance(cur, dict) else None
        # 쌍(dual): main/off 가 같은 이름이면 양손 통째로 벗고 1개만 회수.
        if slot in ("main_hand", "off_hand"):
            other = "off_hand" if slot == "main_hand" else "main_hand"
            other_item = self.equipped.get(other) or {}
            if isinstance(other_item, dict) and other_item.get("name") == name:
                self.equipped[other] = {"name": "", "effect": None}
        self.equipped[slot] = {"name": "", "effect": None}
        self.grant_item(name, effect, 1, kind="equipment")
...
class Monster:
    ...
    def __init__(self, name: str, max_hp: int, speed: int = 10):
        ...
        # 🆕 O-1: 마지막으로 '실제 HP를 깎은' player_id. attackers 는 이제 전투 중 행동자 전원을
        # 담으므로(어시스트용) attackers[-1] 이 처치자가 아닐 수 있다 → DOT 사망 처치자 판정은
        # 이 값 우선. ponytail: save 직렬화 안 함(리로드 후엔 attackers[-1] 폴백, DOT 리로드 드묾).
        self.last_damager_id: Optional[str] = None
...
# 🆕 [L] 공동 낙서판 — 기다리는 시간에 다 같이 그림. LLM 무관 순수 소셜.
DOODLE_MAX_STROKES = 1200          # 획 상한 (초과 시 오래된 획부터 트림)
DOODLE_MAX_POINTS = 30_000         # 총 포인트 상한 (대역폭·메모리)
DOODLE_PTS_PER_STROKE = 256        # 획 하나당 최대 포인트
DOODLE_RATE_WINDOW = 1.0           # 레이트리밋 창(초)
DOODLE_RATE_MAX = 10               # 창 안에서 허용할 최대 획 수 (초과분 silent drop)
DOODLE_COLORS = {"#111111", "#ffffff", "#e23c3c", "#f0a020",
                 "#f5e050", "#3ca85a", "#3a7bd5", "#9b59b6"}  # 화이트리스트 8색
DOODLE_WIDTHS = {3, 8}             # 굵기 2단
# 🆕 dormant(휴면) 관련
...
def _apply_imported_sheet(player: "Player", sheet: dict):
    ...
    if isinstance(eq_in, dict):
        ...
        for slot, val in eq_in.items():
            if slot not in VALID_SLOTS:
                continue
            # O-3: 구 'weapon' 키 → 'main_hand' 마이그레이션 (equipment_bonuses 는 4슬롯만 읽어
            # 'weapon' 에 저장하면 보너스가 증발한다). from_save_dict 와 동일 처리.
            slot = {"weapon": "main_hand"}.get(slot, slot)
            if isinstance(val, dict):
...
class GameRoom:
    def __init__(self, room_id: str, scenario_id: Optional[str] = None):
        ...
        # 🆕 [N-2] 장소별 배경 URL 캐시 (place -> [url..]). 메모리 전용(to_save_dict 제외),
        #   삽입순 상한 30 장소 — 방 나가면 자연 소멸. 같은 장소 재탐색 시 첫 장 재사용(즉시).
        self.explore_url_cache: "Dict[str, List[str]]" = {}
        # 🆕 [L] 공동 낙서판 — 획 리스트. {pid, color, w, pts:[[x,y]..]} 좌표 0..1 정규화.
        #   방 소유(플레이어별 정리 불필요). **휘발성 — to_save_dict 에 넣지 않는다**(대역폭·재시작 시 증발).
        self.doodle: List[dict] = []
        self._doodle_rate: Dict[str, List[float]] = {}  # sender_id -> 최근 획 타임스탬프 (레이트리밋)
    ...
    # ── [L] 공동 낙서판 ─────────────
    def add_doodle_stroke(self, sender_id: str, color, w, pts, now: Optional[float] = None):
        """한 획을 검증·레이트리밋·캡 트림 후 doodle 에 append. 통과하면 stroke dict, 거부/드롭이면 None.
        좌표는 0..1 정규화 float 만 허용(소수 3자리 반올림). WS 핸들러와 단위테스트 공용 진입점."""
        if not sender_id:
            return None
        if color not in DOODLE_COLORS:
            return None
        try:
            w = int(w)
        except (TypeError, ValueError):
            return None
        if w not in DOODLE_WIDTHS:
            return None
        if not isinstance(pts, list) or not (2 <= len(pts) <= DOODLE_PTS_PER_STROKE):
            return None
        clean = []
        for pt in pts:
        ...
        # 폭주 방어 — 직전 1초 내 DOODLE_RATE_MAX 초과분 silent drop
        t = time.time() if now is None else now
        ts = self._doodle_rate.setdefault(sender_id, [])
        ts[:] = [s for s in ts if t - s < DOODLE_RATE_WINDOW]
        if len(ts) >= DOODLE_RATE_MAX:
            return None
        ts.append(t)
        stroke = {"pid": sender_id, "color": color, "w": w, "pts": clean}
        self.doodle.append(stroke)
        while len(self.doodle) > DOODLE_MAX_STROKES:
            self.doodle.pop(0)
        total = sum(len(s["pts"]) for s in self.doodle)
        while total > DOODLE_MAX_POINTS and self.doodle:
            total -= len(self.doodle.pop(0)["pts"])
        return stroke

    def can_clear_doodle(self, pid: Optional[str]) -> bool:
        """전체 지우기는 방장만."""
        return bool(pid) and self.owner_id == pid
    ...
    def _move_to_dormant(self, player_id: str):
        ...
        # _last_chat_at 은 lazy 생성 — 있을 때만 정리.
        if hasattr(self, "_last_chat_at"):
            self._last_chat_at.pop(player_id, None)
    ...
    async def announce_departure(self, player: Player):
        ...
        try:
            ...
            try:
                text, _quality_bad = await _dm_complete_checked(system_prompt, llm_messages,
                                                                max_tokens=300, where="announce_departure")
            ...
            async with self.lock:
                self.messages.append(_dm_assistant_msg(text, _quality_bad))
                self._trim_messages()
        except Exception as e:
    ...
    async def announce_return(self, player: Player, seconds_away: int, is_takeover: bool):
        ...
        try:
            ...
            try:
                text, _quality_bad = await _dm_complete_checked(system_prompt, llm_messages,
                                                                max_tokens=300, where="announce_return")
            except BaseException:
                async with self.lock:
                    self.messages = [m for m in self.messages if m is not user_msg]
                raise
    ...
    def _distribute_kill_xp(self, monster_events: List[dict]) -> List[dict]:
        ...
        for ev in monster_events:
            ...
            # O-1: killer 명시값 우선(그 턴 행동자/실제 딜러). 없으면 attackers[-1] 폴백(구 세이브 등).
            killer_id = ev.get("killer") or attackers[-1]
    ...
    def tick_monsters_round(self) -> List[dict]:
        ...
        for name, monster in list(self.monsters.items()):
            ...
            if monster.hp <= 0:
                # DOT 로 죽음 — 행동자가 없으므로 마지막 '실제 데미지 딜러'가 처치자.
                events.append({
                    "kind": "defeated",
                    "name": monster.name,
                    "max_hp": monster.max_hp,
                    "attackers": list(monster.attackers),
                    "killer": monster.last_damager_id,
                    "by_dot": True,
                })
    ...
    def _llm_slice(self) -> List[dict]:
        ...
        """
        # 패키지 Q: 품질 fail 마킹된 응답은 히스토리에서 제외 → 오염 루프 차단(화면표시는 유지).
        msgs = [m for m in self.messages[-LLM_CONTEXT_WINDOW:] if not m.get("quality_bad")]
    ...
    async def get_dm_intro(self) -> str:
        ...
        text, _quality_bad = await _dm_complete_checked(system_prompt, llm_messages, max_tokens=700,
                                                        where="get_dm_intro")
    ...
    async def process_action(self, player_id: str, action: str) -> Tuple[str, dict]:
        ...
        try:
            text, _quality_bad = await _dm_complete_checked(
                system_prompt, llm_messages, max_tokens=600,
                on_chunk=on_chunk, where="process_action",
            )
        ...
        if exp:
            exp["starter_id"] = player_id  # O-4: 탐색 시작자 = 이 행동을 한 플레이어(중단 권한자)
    ...
    async def maybe_launch_exploration(self) -> Optional[dict]:
        ...
        place = (pend["place"] or "").strip()
        stages = script.get("scene_stages") or []
        cached = self.explore_url_cache.get(place) if place else None  # 빈 장소명은 캐시 오염 방지
        if cached:
            # N-2 같은 장소 재탐색: 캐시 첫 장 재사용(이미 CDN 예열 → 즉시 표시) + 새 시드 변주 1~2장(계속 생성).
            stage_imgs = [cached[0]]
            for s in stages[:2]:
                u = build_scene_image_url(s, seed=random.randint(1, 99999))
                if u:
                    stage_imgs.append(u)
            if len(stage_imgs) < 2:  # 새 묘사 없음 → 캐시 나머지로 보충
                stage_imgs += cached[1:3]
        else:
            stage_imgs = [u for u in (build_scene_image_url(s) for s in stages) if u]
        stage_imgs = list(dict.fromkeys(stage_imgs))[:3]  # 중복 URL 제거(같은 그림 방지) + 상한 3
        # N-2 캐시에는 '진짜 scene_stages 산출 URL' 만 저장 (current_scene_url 폴백은 저장 금지 → 오탐 캐시히트 방지).
        if stage_imgs and place:
            self.explore_url_cache[place] = stage_imgs
            while len(self.explore_url_cache) > 30:
                self.explore_url_cache.pop(next(iter(self.explore_url_cache)), None)
        imgs = stage_imgs
        img = (imgs[0] if imgs else None) or self.current_scene_url
        if not imgs and img:
            imgs = [img]  # 폴백 각본(stages 없음) → 기존 단일 이미지 로직(캐시엔 안 넣음)
        now = time.time()
        ...
        self.exploration = {
            ...
            "gained": [], "starter_id": pend.get("starter_id"),  # O-4: 중단 권한(방장 or 시작자)
        }
        ...
        return {"type": "exploration_start", "place": pend["place"], "danger": pend["danger"],
                ...
                "starter_id": pend.get("starter_id")}  # O-4: 클라 ⏹ 표시 조건(시작자)

    def _explore_trap_save(self, player: "Player", dmg: int) -> dict:
        """함정 스텟 판정. d20 + 능력 보정치 vs DC.
        1) 지혜/지능(높은 쪽)으로 '미리 발견' 성공 → 대미지 0 (완전 회피).
        2) 실패 시 기교로 '회피/낚아챔' 성공 → 대미지 절반.
        반환 {damage, save} — save 는 무마 서술용 {kind, label, value} 또는 None."""
        danger = (self.exploration or {}).get("danger", "중")
        dc = 13 if danger == "상" else 10
        bon = player.equipment_bonuses()
        def eff(key, base):  # 장비 보너스 포함 실효 스텟
            return base + bon.get(key, 0)
        wis, intel, dex = (eff("wisdom", player.wisdom),
                           eff("intelligence", player.intelligence),
                           eff("dexterity", player.dexterity))
        # 1) 미리 발견 — 지혜/지능 중 높은 쪽 (완전 회피)
        perc_val, perc_label = (wis, "지혜") if wis >= intel else (intel, "지능")
        if random.randint(1, 20) + ability_modifier(perc_val) >= dc:
            return {"damage": 0, "save": {"kind": "spot", "label": perc_label, "value": perc_val}}
        # 2) 회피/낚아챔 — 기교 (절반 경감)
        if random.randint(1, 20) + ability_modifier(dex) >= dc:
            return {"damage": max(1, dmg // 2), "save": {"kind": "dodge", "label": "기교", "value": dex}}
        return {"damage": dmg, "save": None}

    def _explore_encounter(self, player: "Player") -> dict:
        """적 조우 진입 판정 — 탐색자 스텟으로 전투 시작 상태를 결정 (d20+보정 vs DC).
        우선순위: 기교=아군 기습 성공 > 지능=약점 간파 > 지혜=매복 간파 > 실패=적의 기습.
        반환 {kind, label, value, status(=[적 상태] 메모), note(=서사 지시)}."""
        danger = (self.exploration or {}).get("danger", "중")
        dc = 13 if danger == "상" else 11
        bon = player.equipment_bonuses()
        def eff(key, base):
            return base + bon.get(key, 0)
        dex = eff("dexterity", player.dexterity)
        intel = eff("intelligence", player.intelligence)
        wis = eff("wisdom", player.wisdom)
        def check(v):
            return random.randint(1, 20) + ability_modifier(v) >= dc
        if check(dex):
            return {"kind": "surprise", "label": "기교", "value": dex,
                    "status": "기습당함 — 첫 턴 행동·반격 불가, 아군 선제 일격 자동 명중",
                    "note": f"{player.name}이(가) 기교({dex})로 적을 먼저 덮쳤다 — 아군 기습 성공."}
        if check(intel):
            return {"kind": "weakness", "label": "지능", "value": intel,
                    "status": "약점 노출 — 아군 첫 공격 명중·피해 유리",
                    "note": f"{player.name}이(가) 지능({intel})으로 적의 약점을 간파했다 — 첫 공격 유리."}
        if check(wis):
            return {"kind": "spotted", "label": "지혜", "value": wis,
                    "status": "",
                    "note": f"{player.name}이(가) 지혜({wis})로 매복을 미리 알아챘다 — 기습 없이 정상 대치."}
        return {"kind": "ambushed", "label": None, "value": None,
                "status": "선제 기습 — 적이 먼저 움직임, 아군 첫 턴 회피 어려움",
                "note": "적이 매복하고 있었다 — 아군이 기습당해 적이 선제한다."}

    def _explore_discovery(self, player: "Player", text: str = "") -> Optional[dict]:
        """심심한 칸(flavor/empty)에서 탐색자 스텟으로 '발견' 판정.
        지혜·지능=통찰, 기교=재빠름, 근력=완력, 건강=끈기 중 가장 높은 능력으로 d20+보정 vs DC15.
        성공 시: 건강이면 소량 HP 회복, 그 외엔 숨겨진 소액 골드. 실패면 None(=평범한 심심한 칸)."""
        bon = player.equipment_bonuses()
        def eff(key, base):
            return base + bon.get(key, 0)
        cands = [  # (실효값, 표시명, 서술종류)
            (eff("wisdom", player.wisdom), "지혜", "insight"),
            (eff("intelligence", player.intelligence), "지능", "insight"),
            (eff("dexterity", player.dexterity), "기교", "nimble"),
            (eff("strength", player.strength), "근력", "force"),
            (eff("constitution", player.constitution), "건강", "vigor"),
        ]
        val, label, kind = max(cands, key=lambda c: c[0])
        if random.randint(1, 20) + ability_modifier(val) < 15:
            return None
        danger = (self.exploration or {}).get("danger", "중")
        if kind == "vigor":  # 건강 → 지치지 않고 스스로 추스름 (탐색자 HP 소량 회복)
            heal_hi = {"하": 6, "중": 8, "상": 10}.get(danger, 8)
            amt = random.randint(3, heal_hi)
            before = player.hp
            player.hp = min(player.max_hp, player.hp + amt)
            return {"type": "discovery", "kind": kind, "label": label, "value": val,
                    "heal": player.hp - before, "hp": player.hp, "max_hp": player.max_hp,
                    "who": player.name, "text": text}
        # 통찰/재빠름/완력 → 숨겨진 소액 골드 (파티 공유)
        gold_hi = {"하": 8, "중": 14, "상": 22}.get(danger, 14)
        amt = random.randint(3, gold_hi)
        gold_events = []
        for p in self.players.values():
            if not p.is_alive():
                continue
        ...
        return {"type": "discovery", "kind": kind, "label": label, "value": val,
                "amount": amt, "gold_events": gold_events, "who": player.name, "text": text}

    def _resolve_explore_cell(self, player: "Player", cell: dict) -> dict:
        """칸 1개의 효과 적용 (동기). 기존 아이템/골드/HP/몬스터 경로 재사용. 이벤트 dict 반환."""
        t = cell.get("type")
        if t == "flavor":
            return self._explore_discovery(player, cell.get("text", "")) \
                or {"type": "flavor", "text": cell.get("text", "")}
        if t == "item":
            name = cell.get("name", "")
            slot = cell.get("slot")
        ...
        if t == "trap":
            ...
            save = self._explore_trap_save(player, dmg)  # 스텟 판정 — 대미지 경감/회피
            before = player.hp
            player.hp = max(1, player.hp - save["damage"])  # 탐색으론 사망 금지 (최소 1 HP)
        ...
        if t == "enemy":
            return {"type": "enemy", "name": cell.get("name"), "hp": cell.get("hp"), "enemy": True,
                    "encounter": self._explore_encounter(player)}
        return self._explore_discovery(player) or {"type": "empty"}
    ...
    def finalize_exploration(self, end_reason: str, enemy_cell: Optional[dict] = None) -> dict:
        ...
        if end_reason == "enemy" and enemy_cell:
            ...
            enc = enemy_cell.get("encounter") or {}
            tag = f"[적 등장: {ename} | HP {ehp}]"
            if enc.get("status"):  # 진입 상태 메모 — 같은 파싱 호출서 등장 뒤 적용됨
                tag += f"\n[적 상태: {ename} | {enc['status']}]"
            payload["monster_events"] = parse_and_apply_monsters(tag, self.monsters)
            payload["enemy_name"] = ename
            enc_note = enc.get("note") or ""
            note = f"[시스템] 탐색 중 '{place}'에서 {ename}(HP {ehp})과(와) 조우 — {enc_note} 전투가 시작된다. 획득: {summary}."
        else:
    ...
    def exploration_public(self) -> Optional[dict]:
        ...
        return {"place": exp["place"], "danger": exp["danger"], "pos": exp["pos"],
                ...
                "starter_id": exp.get("starter_id")}  # O-4: 재접속 복원 시에도 시작자 중단권 유지
    ...
    async def process_monster_turn(self, monster_name: str) -> Tuple[Optional[str], dict]:
        ...
        try:
            text, _quality_bad = await _dm_complete_checked(system_prompt, llm_messages,
                                                            max_tokens=350, where="process_monster_turn")
        except BaseException:
            async with self.lock:
                self.messages = [m for m in self.messages if m is not user_msg]
            raise
...
async def _send_doodle_state(ws: WebSocket, room: "GameRoom"):
    """🆕 [L] 입장/재입장/관전/이어받기 직후 현재 낙서판 1회 복원. 비어있으면 생략."""
    if room.doodle:
        try:
            await ws.send_json({"type": "doodle_state", "strokes": room.doodle})
        except Exception:
            pass
    ...
    try:
        while True:
            ...
            if msg == "create_room":
                ...
                await _send_doodle_state(websocket, room)  # 🆕 [L] 낙서판 복원
                await room.broadcast({
                    "type": "player_joined",
            ...
            elif msg == "doodle_stroke":
                # 🆕 [L] 공동 낙서판 — 한 획 수신. 플레이어·관전자 모두 허용(채팅과 동일 규칙).
                if not current_room or current_room not in rooms:
                    continue
                room = rooms[current_room]
                # sender_id: 플레이어 우선, 없으면 관전자. 둘 다 아니면 권한 없음.
                if player_id and player_id in room.players:
                    d_sender = player_id
                elif spectator_id and spectator_id in room.spectators:
                    d_sender = spectator_id
                else:
                    continue
                # 검증·레이트리밋·캡 트림은 add_doodle_stroke 안. 거부/드롭이면 None → silent.
                stroke = room.add_doodle_stroke(
                    d_sender, data.get("color"), data.get("w"), data.get("pts"))
                if stroke is None:
                    continue
                # 보낸 사람 제외 브로드캐스트 (로컬 즉시 렌더 중복 방지). exclude 는 관전자에도 적용됨.
                await room.broadcast({"type": "doodle_stroke", "stroke": stroke}, exclude=d_sender)

            elif msg == "doodle_clear":
                # 🆕 [L] 전체 지우기 — 방장만.
                if not current_room or current_room not in rooms:
                    continue
                room = rooms[current_room]
                if not room.can_clear_doodle(player_id):
                    await _send_error(websocket, "방장만 낙서판을 지울 수 있습니다.")
                    continue
                room.doodle.clear()
                await room.broadcast({"type": "doodle_clear"})
                ...
                if action == "equip" or (action == "" and kind == "equipment"):
                    ...
                    if not player.is_alive():
                        await _send_error(websocket, "사망 상태에서는 장비를 바꿀 수 없습니다.")
                        continue
            ...
            elif msg == "unequip_slot":
                # 🆕 [P-2] 슬롯 장비를 벗어 소지품으로 회수. data: { slot }
                if not current_room or current_room not in rooms or not player_id:
                    continue
                room = rooms[current_room]
                player = room.players.get(player_id)
                ...
                if not player.is_alive():
                    await _send_error(websocket, "사망 상태에서는 장비를 바꿀 수 없습니다.")
                    continue
                raw_slot = str(data.get("slot", "")).strip().lower()
                slot = _SLOT_ALIASES.get(raw_slot, raw_slot)
                removed = player.unequip_slot(slot)
                if not removed:
                    continue  # 빈 슬롯 no-op — 조용히 무시.
                player.pending_notes.append(
                    f"※ {player.name}이(가) '{removed['name']}'을(를) 벗어 소지품에 넣었다.")
                await room.broadcast({
                ...
                # 🆕 탐색을 즉시 중단 — 몬스터 등록 없이 종료(획득물은 유지).
                # O-4: 중단 허용 = 방장 또는 탐색 시작자.
                if not current_room or current_room not in rooms or not player_id:
                    continue
                room = rooms[current_room]
                # 권한 검사와 finalize 를 같은 lock 안에서 원자 처리 — lock 밖에서 starter_id 를 먼저 읽으면
                # await 사이에 새 탐색으로 교체돼 오래된 시작자가 다른 탐색을 중단시키는 레이스가 생긴다.
                async with room.lock:
                    exp = room.exploration
                    starter_id = (exp or {}).get("starter_id")
                    if room.owner_id != player_id and starter_id != player_id:
                        await _send_error(websocket, "방장 또는 탐색을 시작한 사람만 중단할 수 있습니다.")
                        continue
                    ...
                    if tap and tap["ended"]:
                        ...
                        if tap["end_reason"] == "enemy":
                            enemy_cell = {"name": tap["event"].get("name"), "hp": tap["event"].get("hp"),
                                          "encounter": tap["event"].get("encounter")}
                ...
                # O-4: 말로 하는 탐색 시작도 같은 진입 경로 재사용(_is_explore_action 로 확장).
                # 진입 불가 상태(전투 중·이미 탐색 중)면 조건 미충족으로 조용히 일반 LLM 경로 폴백.
                if (_is_explore_intent_action(action_text) or _is_explore_action(action_text)) \
                        and not any(m.hp > 0 for m in room.monsters.values()) \
                ...
                # 🆕 매복/기습 의도 → 서버가 기교(DEX) 판정을 미리 굴려 결과를 DM 에 확정 주입.
                # (LLM 재량에 맡기지 않고 스텟이 반드시 반영되게 — 탐색 미니게임과 동일 철학.)
                if _is_ambush_action(action_text):
                    dex = player.dexterity + player.equipment_bonuses().get("dexterity", 0)
                    roll = random.randint(1, 20)
                    mod = ability_modifier(dex)
                    total = roll + mod
                    if total >= 12:
                        player.pending_notes.append(
                            f"※ {player.name}의 매복/기습 기교 판정 성공 (기교 {dex}, d20:{roll}{mod:+d}={total} ≥ 12). "
                            f"본문에 판정을 `[🎲DM d20: {roll}] {mod:+d}` 로 노출하고, 기습 성공을 반영하라 — "
                            f"첫 일격은 자동 명중·추가 피해로 유리하게, 적은 이번 턴 제대로 반격하지 못한다."
                        )
                    else:
                        player.pending_notes.append(
                            f"※ {player.name}의 매복/기습 기교 판정 실패 (기교 {dex}, d20:{roll}{mod:+d}={total} < 12). "
                            f"`[🎲DM d20: {roll}] {mod:+d}` 노출 후, 기습이 어설퍼 적이 눈치챘음을 반영하라 — "
                            f"기습 보너스 없이, 오히려 적이 경계·선제할 수 있다."
                        )

                # 🆕 O-1: 전투 중이면 이번 행동자를 살아있는 모든 몬스터의 기여자로 기록.
                # 공격·치료·지원 불문 — 협동 게임이라 힐러/버퍼도 어시스트 XP를 받는 게 의도된 설계.
                # (실제 데미지는 별도 last_damager_id 로 추적하므로 처치자 판정은 오염되지 않는다.)
                for _mon in room.monsters.values():
                    if _mon.hp > 0:
                        _mon.note_attacker(player_id)
                ...
                if room.is_tpk():
                    ...
                    try:
                        ...
                        try:
                            tpk_text, _tpk_quality_bad = await _dm_complete_checked(
                                system_prompt, llm_messages, max_tokens=500, where="tpk")
                        except BaseException:
                            async with room.lock:
                        ...
                        async with room.lock:
                            room.messages.append(_dm_assistant_msg(tpk_text, _tpk_quality_bad))
