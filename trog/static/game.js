...
function sendAction() {
  ...
  if (_amIDead()) {
    sysMsg('사망 상태 — 행동 불가. 동료가 부활시켜주거나 구원의 빛이 임할 때까지 대기.');
    return;
  }
...
(function bindReconnectTriggers() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
      ...
      const suspectZombie = hiddenMs >= MOBILE_BG_ZOMBIE_THRESHOLD_MS;
...
function showCampaignEnding(ending) {
  ...
  const title = `${ending.scenario_emoji || '📜'} ${escapeHtml(ending.scenario_name || '캠페인 종료')}`;
