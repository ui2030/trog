...
## V7 — 2026-04-28 (디테일 연결성 재검사 — 서사 ↔ 시스템 어긋남 5건 해소)

사용자 피드백: 쌍단검 2회 공격 X / 퀘스트 완료해도 인벤 잔존 / 방패 막기 판정 없음 / 강화 후 무기 이름 그대로 / 상점 구매 시 인벤 누락. 모두 **DM 이 시스템 태그를 누락**해 발생. 시스템 1건 신규 + 프롬프트 가이드 5건 보강. 자세한 진단·셀프 리뷰: [V7_CONNECTIVITY_FIX.md](V7_CONNECTIVITY_FIX.md).

### 🔧 시스템 (1건 신규)

- **`[이름 장비 강화: 슬롯 | 새 이름 | 새 효과]` 태그 추가**. 슬롯 atomic 교체 + dual-wield 자동 동기화 + 빈 슬롯 가드 + 효과 미지정 시 기존 보존. 클라엔 `events.equipment_upgrades` 로 노출, 토스트 `⚒ 이름 슬롯 (양손) 강화: 이전 → 새 이름`.
- 이전 강화 흐름은 `[장비 해제: weapon]` + `[획득: 새 이름 | 무기 | ...]` 두 줄이라 DM 이 절반만 찍는 사고가 잦았음. 한 줄로 통합.

### 📝 DM 프롬프트 가이드 (5건 보강 — LLM 톤이라 100% 보장 X)

[main.py `DM_SYSTEM_PROMPT`](../main.py) "장비 해제" 섹션 직후에 5개 V7 블록 추가:

1. **장비 강화 / 업그레이드 / 리네임** — 새 태그 사용법 + 흔한 실수 + 양손 동기화
2. **양손 무기 dual-wield 공격 판정** — d20 두 번, 각각 명중·실패 별도 묘사
3. **방패 막기 판정** — d20 추가 굴림 (≥15 -80% / ≥10 -50% / <10 정상). 마법·범위·측면은 막기 어려움
4. **🚨 절대 규칙 — 상점·거래 양쪽 태그** — `[골드 -N]` + `[획득:]` 동시. 한쪽만 찍으면 desync 명시
5. **퀘스트 완료 인벤 제거** — `[이름 사용: 퀘스트아이템]` + 건네는 장면 묘사 + 보상 태그 세트

### 🧪 검증

- `python -m py_compile main.py` ✅
- `node --check static/game.js` ✅
- 정규식 충돌 검토 — HP/MP/XP/골드/획득/사용/장비해제/버프 모두 다른 키워드라 충돌 없음 ✅
- 엣지 — 빈 슬롯 무시 + 효과 미지정 보존 + dual-wield 멱등성 ✅

### ⚠ 한계

- 양손 d20 두 번 / 방패 막기 판정 / 상점 양쪽 태그 / 퀘스트 사용 태그 — 전부 **DM 컴플라이언스에 달림**. 시스템 강제 X. 누락 시 자동 복구 메커니즘 없음.
- 어긋남 잦으면 V7 가이드를 더 단호하게(`반드시`, `금지` 강조) 강화하는 방향.
- DM 프롬프트 길이가 V7 로 ~80줄 증가 → prefill 시간·토큰 비용 증가. 모델 변경 시 토큰 한도 재점검.
